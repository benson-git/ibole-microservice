...
public abstract class ServiceRegistryProvider {
	...
	private static final ServiceRegistryProvider provider = load(Thread
			.currentThread().getContextClassLoader());

	static final ServiceRegistryProvider load(ClassLoader cl) {
		ServiceLoader<ServiceRegistryProvider> providers = ServiceLoader.load(
				ServiceRegistryProvider.class, cl);
