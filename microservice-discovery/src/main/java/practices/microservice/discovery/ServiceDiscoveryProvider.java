...
public abstract class ServiceDiscoveryProvider {
	...
	private static final ServiceDiscoveryProvider provider = load(Thread
			.currentThread().getContextClassLoader());

	static final ServiceDiscoveryProvider load(ClassLoader cl) {
		ServiceLoader<ServiceDiscoveryProvider> providers = ServiceLoader.load(
				ServiceDiscoveryProvider.class, cl);
